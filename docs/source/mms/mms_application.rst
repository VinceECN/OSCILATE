...
Note the dependency of :math:`f_{A_i}^{(j)}` on :math:`t_1` due to the possible presence of forcing. The complex modulation equations are derived in :func:`secular_analysis`.
...
The polar coordinates are introduced in :func:`polar_coordinates`. The real modulation equations are only computed for the autonomous system, as described below. 
