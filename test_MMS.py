...
# # Parameters and variables
# omega0, omega1, F, mu0, mu1 = symbols(r'\omega_0, \omega_1, F, \mu_0, \mu_1', real=True, positive=True)
...
# Eq1 = x1.diff(t,2) + omega1**2*x1 + 2*mu1*x1.diff(t) + sum([alphas[4+ii] * x0**(3-ii)*x1**ii for ii in range(4)])
# fcoeff = [Gam0, Gam1]
...
# param_to_scale = (*alphas,          F, mu0, mu1, sigma1)
...
# Dbeta         = symbols(r"\Delta\beta", real=True) # Phase difference beta1-beta0
...
# Eq1 = x1.diff(t,2) + omega1**2*x1 + 2*mu1*x1.diff(t) + alpha1*x0**2
# fcoeff = [eta0, eta1]
# dyn = MMS.Dynamical_system(t, [x0, x1], [Eq0, Eq1], [omega0, omega1], f_coeff=fcoeff, F=F)
...
# param_to_scale = (F, eta0, eta1, mu0, mu1, sigma1, alpha0, alpha1)
...
# dic_fbeta1 = fbeta[1].collect(cos(beta[1]), evaluate=False)
# Eq_a0      = (((dic_fa1[1]/dic_fa1[sin(beta[1])])**2 + (dic_fbeta1[1]/dic_fbeta1[cos(beta[1])])**2 - 1).subs(sub_Dbeta).subs(ss.coord.a[1]**2, a1_2_sol)).simplify()
